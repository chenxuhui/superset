    ...
    const currentFilterLabels = []
      .concat(
        isSingleValue
          ? activeKey
          : activeKey
              .substring(activeKey.indexOf('['), activeKey.indexOf(']'))
              .split(','),
      )
      .map(key => {
        const [, column] = getDashboardFilterByKey(key);
        return column;
      })
      .join(',');
      ...
      <React.Fragment>
        <div className="filter-scope-container">
          <div className="filter-scope-header">
            <h4>
              <span>{t('Configure filter scopes: ')}</span>
              <span className="selected-scopes">{currentFilterLabels}</span>
            </h4>
          </div>
